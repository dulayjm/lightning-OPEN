...
# neptune.create_experiment(name='iNat_Plants_only', params={'lr': 0.0045}, tags=['resnet', 'iNat'])
...
def load_taxonomy(ann_data, tax_levels, classes):
    ...
    if 'categories' in ann_data.keys():
        num_classes_ = len(ann_data['categories'])
        # print("Num classes", num_classes)
        for tt in tax_levels:
            tax_data = [aa[tt] for aa in ann_data['categories']]
            _, tax_id = np.unique(tax_data, return_inverse=True)
            taxonomy[tt] = dict(zip(range(num_classes_), list(tax_id)))
    else:
        # set up dummy data
        for tt in tax_levels:
...
class INAT(torch.utils.data.Dataset):
    def __init__(self, root, ann_file, is_train=True):
        ...
        self.imgs = [aa['file_name'] for aa in ann_data['images'] if 'Plants' in aa['file_name']]
        self.ids = [aa['id'] for aa in ann_data['images'] if 'Plants' in aa['file_name']]
        ...
        values = self.classes_taxonomic.values()
        values = list(values)
        plants = [v for v in values if v[6] == 0]
        # print('plants classes taxanomic', plants)
        # print('len of plants set', len(plants)) # 682 - this is the correct number
        # print('sample image name', self.imgs[2])
        # self.classes = [v[0] for v in plants]
        print('len of classes set', len(self.classes))
        print('len of self.imgs', len(self.imgs))

        # now we must trim the dataset constructed here to just contain the plants from 
        # these annotations sorts of things ...
        # big change to note is if we change self.classes to just the ones in plants,
        # or if using the subset of the identified imgs is enough ...
